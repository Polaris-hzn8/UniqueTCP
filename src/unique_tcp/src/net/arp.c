...
static void arp_pkt_display(arp_pkt_t* packet) {
  ...
  dbg_dump_hwaddr(DBG_ARP, "        mac: ", packet->sender_hwaddr, ETHER_HWA_SIZE);
  dbg_dump_ip_buf(DBG_ARP, "\n     target: ", packet->target_paddr);
  dbg_dump_hwaddr(DBG_ARP, "        mac: ", packet->target_hwaddr, ETHER_HWA_SIZE);
  plat_printf("\n------------ arp packet end -----------\n");
}
#else
...
static net_err_t is_pkt_ok(arp_pkt_t* arp_packet, uint16_t size, netif_t* netif) {
  ...
  if ((x_ntohs(arp_packet->htype)) != ARP_HW_ETHER || (arp_packet->hwlen != ETHER_HWA_SIZE) ||
...
net_err_t arp_resolve(netif_t* netif, const ipaddr_t* ipaddr, pktbuf_t* buf) {
  ...
  if (entry) {
    ...
    cache_entry_set(entry, empty_hwaddr, ipaddr->a_addr, netif, NET_ARP_WAITING);
    nlist_insert_first(&cache_list, &entry->node);
