...
static void display_raw_list(void) {
  ...
  nlist_for_each(node, &raw_list) {
    ...
    plat_printf("[%d]:\n", idx++);
    dbg_dump_ip(DBG_RAW, "    local: ", &raw->base.local_ip);
    dbg_dump_ip(DBG_RAW, "    remote: ", &raw->base.remote_ip);
    plat_printf("\n");
  }
}
#else
...
static net_err_t raw_sendto(struct _sock_t* sock,
                            const void* buf,
                            size_t len,
                            int flags,
                            const struct x_sockaddr* dest,
...
net_err_t raw_recvfrom(struct _sock_t* sock,
                       void* buf,
                       size_t len,
                       int flags,
...
net_err_t raw_connect(sock_t* s, const struct x_sockaddr* addr, const x_socklen_t addr_len) {
  sock_connect(s, addr, addr_len);
  display_raw_list();
  return NET_ERR_OK;
}

net_err_t raw_bind(sock_t* s, const struct x_sockaddr* addr, x_socklen_t len) {
