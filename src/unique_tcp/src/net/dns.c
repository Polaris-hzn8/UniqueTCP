...
void dns_init(void) {
  ...
  nlist_init(&req_list);
  mblock_init(&req_mblock, dns_req_tbl, sizeof(dns_req_t), DNS_REQ_SIZE, NLOCKER_THREAD);

  net_timer_add(&entry_update_timer, "dns timer", dns_update_tmo, (void*)0, DNS_UPDATE_PERIOD * 1000, NET_TIMER_RELOAD);
...
static net_err_t dns_send_query(dns_req_t* req) {
  ...
  buf = add_query_field(req->domain_name, buf, sizeof(working_buf) - (buf - working_buf));
  ...
  return udp_sendto(
    (sock_t*)dns_udp, working_buf, buf - working_buf, 0, (const struct x_sockaddr*)&dest, sizeof(dest), (ssize_t*)0);
}
...
void dns_in(void) {
  ...
  net_err_t err =
    udp_recvfrom((sock_t*)dns_udp, working_buf, sizeof(working_buf), 0, (struct x_sockaddr*)&src, &addr_len, &rcv_len);
  if (err < 0) {
  ...
  nlist_for_each(curr, &req_list) {
    ...
    if (dns_hdr->qdcount == 1) {
      ...
      rcv_start = domain_name_cmp(req->domain_name, (const char*)rcv_start, rcv_end - rcv_start);
    ...
    for (int i = 0; (i < dns_hdr->ancount) && (rcv_start < rcv_end); i++) {
      ...
      if ((af->class == x_ntohs(DNS_QUERY_ClASS_INET)) && (af->type == x_ntohs(DNS_QUERY_TYPE_A)) &&
