...
int ipaddr_is_match(const ipaddr_t* dest_ip, const ipaddr_t* src_ip, const ipaddr_t* netmask) {
  ...
  if (ipaddr_is_direct_broadcast(dest_ip, netmask) && ipaddr_is_equal(&dest_netid, &src_netid)) {
    return 1;
  }
