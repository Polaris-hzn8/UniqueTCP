...
net_err_t sock_create_req_in(func_msg_t* msg) {
  ...
  if (param->type < 0 || (param->type >= sizeof(sock_info_tbl) / sizeof(sock_info_tbl[0]))) {
...
net_err_t sock_sendto_req_in(struct _func_msg_t* msg) {
  ...
  net_err_t err =
    sock->ops->sendto(sock, data->buf, data->len, data->flags, data->addr, *data->addr_len, (ssize_t*)&data->comp_len);
...
net_err_t sock_recvfrom_req_in(struct _func_msg_t* msg) {
  ...
  net_err_t err =
    sock->ops->recvfrom(sock, data->buf, data->len, data->flags, data->addr, data->addr_len, (ssize_t*)&data->comp_len);
