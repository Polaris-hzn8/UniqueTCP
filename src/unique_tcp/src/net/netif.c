...
static const link_layer_t* link_layers[NET_TYPE_SIZE];
...
void display_netif_list(void) {
  ...
  nlist_for_each(node, &netif_list) {
    ...
    dbg_dump_hwaddr(DBG_NETIF, "hwaddr: ", netif->hwaddr.addr, netif->hwaddr.len);
    dbg_dump_ip(DBG_NETIF, " ip: ", &netif->ipaddr);
    dbg_dump_ip(DBG_NETIF, " netmask: ", &netif->netmask);
    dbg_dump_ip(DBG_NETIF, " gateway: ", &netif->gateway);
    plat_printf("\n");
  }
}
#else
