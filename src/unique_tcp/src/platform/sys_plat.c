...
sys_thread_t sys_thread_create(sys_thread_func_t entry, void* arg) {
  ...
  // 初始化系统线程
  int err = task_init(
    &task->task, "net task", TASK_FLAG_SYSTEM, (uint32_t)entry, (uint32_t)(task->stack + sizeof(task->stack)));
  if (err < 0) {
...
sys_thread_t sys_thread_create(void (*entry)(void* arg), void* arg) {
  ...
  int err = pthread_create(&pthread, NULL, (void* (*)(void* arg))entry, arg);
  if (err) {
    return (pthread_t)0;
  }
